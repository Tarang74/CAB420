...
Now that we can mathematically describe a neural network, let us
discuss how a neural network is trained. As with the models discussed
so far, the goal of training a neural network is to find the optimal
weights and biases in each layer that minimise a loss function. This is
done using a technique called \textbf{backpropagation}.
\subsubsection{Loss Functions}
The choice of loss function is important, as it determines how a
network is trained. Below is a table of common loss functions used to
...
\begin{table}[H]
    ...
    % \caption{} % \label{}
\end{table}
\noindent The following graphs show the shape of common loss functions corresponding
to a prediction \(z\) for a true value \(y\).
\begin{aside}
    \textbf{Mean Squared Error \& Mean Absolute Error Loss:}
    \begin{equation*}
        {\left( y - z \right)}^2 \qquad \text{and} \qquad \abs*{y - z}
    \end{equation*}
    for \(y,z \in \R\).
    \begin{figure}[H]
        \centering
...
The choice of loss function depends on the encoding of the target variable.
\begin{itemize}
