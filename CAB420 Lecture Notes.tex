...
that are organised into layers. The simplest form of a neural network
is the \textbf{feedforward neural network}, where data flows from one
layer to the next.
\begin{itemize}
...
\subsection{Early Models of Neural Networks}
...
The choice of loss function is important, as it determines how a
...
would be able to learn complex patterns in data. Below is a table of
common activation functions applied to the weighted sum \(z\).
\begin{table}[H]
    \centering
    \begin{tabular}{lc}
        \toprule
...
overfitting.
\subsection{Considerations for Training Neural Networks for Classification}
...
The following sections discuss various techniques used to train neural
networks.
\subsection{Gradient Descent}
Gradient descent is an iterative optimisation algorithm used to
minimise a function, when a closed-form derivative for a function is
not obtainable. The algorithm works by repeatedly taking steps in the
opposite direction of the gradient of the loss function, as this is the
direction of greatest decrease. The resulting update rule for the
weights of a model is given by:
\begin{equation*}
    w \leftarrow w - \alpha \pdv{L\left( w \right)}{w}
\end{equation*}
for a learning rate \(\alpha\). This learning rate is a hyperparameter
that controls the size of the step taken in the direction of greatest
decrease. If the learning rate is too small, the algorithm may take a
long time to converge, and if it is too large, the algorithm may
oscillate around a minimum.
...
\begin{itemize}
    ...
    \item \textbf{Stochastic Gradient Descent}: Weights are updated after
          computing the gradient of the loss function with respect to a
          single training example:
          \begin{equation*}
              w \leftarrow w - \alpha \pdv{L^{\left( i \right)}\left( w \right)}{w}.
          \end{equation*}
    \item \textbf{Mini-Batch Gradient Descent}: Weights are updated after
          computing the gradient of the loss function with respect to a
          small subset of the training set:
          \begin{equation*}
              \symbf{w} \leftarrow \symbf{w} - \alpha \pdv*{\left( \frac{1}{b} \sum_{i = 1}^b L^{\left( i \right)}\left( \symbf{w} \right) \right)}{\symbf{w}},
          \end{equation*}
          for a batch size \(b\).
\end{itemize}
\subsection{Backpropagation}
As we saw previously, the output of a neural network consists of a
composition of functions, where each layer may have a different number
of nodes. Hence, let use the gradient descent algorithm to optimise the
weights in a network. Let us first define the \textbf{error} of a
neuron in the \(L\)-th layer as:
\begin{equation*}
...
\begin{enumerate}
    ...
    \item Combine the losses over a batch of observations to calculate
          the cost.
    \item Update the weights and biases using the gradient descent
          update rule.
\end{enumerate}
\subsection{Vectorised Backpropagation}
We can vectorise the equations above by denoting the vector of errors
in the \(\ell\)-th layer with \(\symbf{\delta}^{\left[ \ell \right]} \in \R^{1 \times n^{\left[ \ell \right]}}\),
defined as:
\begin{equation*}
